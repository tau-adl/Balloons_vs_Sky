...
class DeepGoogLeNet:
	...
	def inception_module(x, num1x1, num3x3Reduce, num3x3,
		...
		first = DeepGoogLeNet.conv_module(x, num1x1, 1, 1,
		...
		second = DeepGoogLeNet.conv_module(x, num3x3Reduce, 1, 1,
			(1, 1), chanDim, reg=reg, name=stage + "_second1")
		second = DeepGoogLeNet.conv_module(second, num3x3, 3, 3,
		...
		third = DeepGoogLeNet.conv_module(x, num5x5Reduce, 1, 1,
			(1, 1), chanDim, reg=reg, name=stage + "_third1")
		third = DeepGoogLeNet.conv_module(third, num5x5, 5, 5,
		...
		fourth = DeepGoogLeNet.conv_module(fourth, num1x1Proj,
	...
	def build(width, height, depth, classes, reg=0.0005):
		...
		x = DeepGoogLeNet.conv_module(inputs, 64, 5, 5, (1, 1),
			chanDim, reg=reg, name="block1")
		x = MaxPooling2D((3, 3), strides=(2, 2), padding="same",
			name="pool1")(x)
		x = DeepGoogLeNet.conv_module(x, 64, 1, 1, (1, 1),
			chanDim, reg=reg, name="block2")
		x = DeepGoogLeNet.conv_module(x, 192, 3, 3, (1, 1),
			chanDim, reg=reg, name="block3")
		x = MaxPooling2D((3, 3), strides=(2, 2), padding="same",
			name="pool2")(x)

		# apply two Inception modules followed by a POOL
		x = DeepGoogLeNet.inception_module(x, 64, 96, 128, 16,
			32, 32, chanDim, "3a", reg=reg)
		x = DeepGoogLeNet.inception_module(x, 128, 128, 192, 32,
			96, 64, chanDim, "3b", reg=reg)
		x = MaxPooling2D((3, 3), strides=(2, 2), padding="same",
			name="pool3")(x)

		# apply five Inception modules followed by POOL
		# x = DeepGoogLeNet.inception_module(x, 192, 96, 208, 16,
		...
		x = AveragePooling2D((4, 4), name="pool4")(x)
		...
		# create the model
		model = Model(inputs, x, name="DeepGoogLeNet")
